...
!MODULE MODULE_MP_P3   !WRF
 ...
 private :: gamma,derf,find_lookupTable_indices_1,find_lookupTable_indices_2,            &
            ...
            impose_max_total_Ni,check_values
...
! read in ice microphysics table
 ...
 print*
 print*, ' P3 microphysics, version: 2.2.4'
...
!==
 ...
 SUBROUTINE mp_p3_wrapper_wrf(th_3d,qv_3d,qc_3d,qr_3d,qnr_3d,                            &
                              ...
                              qi2_3d,qni2_3d,qir2_3d,qib2_3d)
  ...
  !      endif
      ...
      call P3_MAIN(qc_3d(its:ite,kts:kte,j),qr_3d(its:ite,kts:kte,j),qnr_3d(its:ite,kts:kte,j),        &
              ...
              birim(its:ite,kts:kte,1:n_iceCat),W(its:ite,kts:kte,j),P(its:ite,kts:kte,j),             &
...
!--- FUTURE version of mp_p3_wrapper_gem:
!    (for radiation interface with multiple ice categories)

! ! !  SUBROUTINE mp_p3_wrapper_gem(qvap,temp,dt,ww,psfc,sigma,kount,trnch,ni,nk,prt_liq,      &
! ! !                               prt_sol,diag_Zet,diag_Zec,diag_effc,diag_vmi,              &
! ! !                               diag_di,diag_rhopo,qc,qr,nr,n_iceCat,                      &
! ! !                               qitot_1,qirim_1,nitot_1,birim_1,diag_effi1,                &
! ! !                               qitot_2,qirim_2,nitot_2,birim_2,diag_effi2,                &
! ! !                               qitot_3,qirim_3,nitot_3,birim_3,diag_effi3,                &
! ! !                               qitot_4,qirim_4,nitot_4,birim_4,diag_effi4)
! ! !
! ! ! !------------------------------------------------------------------------------------------!
! ! ! ! This wrapper subroutine is the main GEM interface with the P3 microphysics scheme.  It   !
! ! ! ! prepares some necessary fields (converts temperature to potential temperature, etc.),    !
...
! ! !  real, intent(out),   dimension(ni,nk)  :: diag_rhopo            ! bulk density, ice                   kg m-3
! ! !  real, intent(out),   dimension(ni,nk)  :: diag_effc             ! effective radius, cloud             m
! ! !  real, intent(out),   dimension(ni,nk)  :: diag_effi1            ! effective radius, ice category 1    m
! ! !  real, intent(out),   dimension(ni,nk), optional  :: diag_effi2  ! effective radius, ice category 2    m
! ! !  real, intent(out),   dimension(ni,nk), optional  :: diag_effi3  ! effective radius, ice category 3    m
! ! !  real, intent(out),   dimension(ni,nk), optional  :: diag_effi4  ! effective radius, ice category 4    m
! ! !
! ! ! !----------------------------------------------------------------------------------------!
! ! !
! ! ! !----- local variables and parameters:
...
! ! !  real, dimension(ni,nk,n_iceCat)  :: nitot      ! ice mixing ratio, number                #  kg-1
! ! !  real, dimension(ni,nk,n_iceCat)  :: birim      ! ice mixing ratio, volume                m3 kg-1
! ! !  real, dimension(ni,nk,n_iceCat)  :: diag_effi  ! ice effective radius                    m
! ! !
! ! !  real, dimension(ni,nk)  :: theta               ! potential temperature                   K
! ! !  real, dimension(ni,nk)  :: pres                ! pressure                                Pa
! ! !  real, dimension(ni,nk)  :: rho_air             ! air density                             kg m-3
...
!==========================================================================================!
 ...
 SUBROUTINE P3_MAIN(qc,qr,nr,th,qv,dt,qitot,qirim,nitot,birim,uzpl,pres,dzq,it,pcprt_liq,  &
                    pcprt_sol,its,ite,kts,kte,nk_bottom,nCat, diag_ze,diag_effc,diag_effi, &
...
!real, intent(inout), dimension(its:ite,kts:kte)      :: nc         ! cloud, number mixing ratio       #  kg-1  (nc is specified in this version)
...
!----- Local variables and parameters:  -------------------------------------------------!
 ...
 real, dimension(its:ite,kts:kte) :: nc    ! cloud, number mixing ratio  # kg-1 (specified in this version))
 real, dimension(its:ite,kts:kte) :: mu_r  ! shape parameter of rain
 real, dimension(its:ite,kts:kte) :: ssat  ! supersaturation w.r.t. liquid
...
!  (all N process rates in # kg-1)
 ...
 real, dimension(kts:kte) :: dum_qit,dum_qr,dum_nit,dum_qir,dum_bir,dum_zit,dum_nr,      &
            ...
            tend_nr,tend_qit,tend_qir,tend_bir,tend_nit
          ! tend_nc,tend_zit
 ...
 logical :: log_nucleationPossible,log_hydrometeorsPresent,log_predictsSsat,log_tmp1,    &
...
! deltaD_init = dummy_in   !temporary; passed in from cld1d
 ...
 log_predictsSsat = .false.   ! for prediction of supersaturation
...
!rhorime_r  = 400.
 ...
 t  = th*(pres*1.e-5)**(rd*inv_cp)  !compute air temperature from potential temperature
 qv = max(qv,0.)                    !clip water vapor to prevent negative values passed in
 ...
 i_loop_main: do i = its,ite  ! main i-loop (around the entire scheme)
    ...
    k_loop_1: do k = kbot,ktop,kdir
     ...
     !calculate temperature from theta
       t(i,k) = th(i,k)*(pres(i,k)*1.e-5)**(rd*inv_cp)   ! ** can be removed, since calc is above (for check_values)
    ...
    if (debug_ON) call check_values(qv,T,qc,qr,nr,qitot,qirim,nitot,birim,i,it,.true.,debug_ABORT,200)
...
! due to vertical motion which IS included
       ...
       if (t(i,k).lt.273.15) then
          aaa = -dum - dqsdt*(-uzpl(i,k)*g*inv_cp)-(qvs(i,k)-dumqvi)*(1.+xxls(i,k)*      &
                inv_cp*dqsdt)*oabi*epsi_tot
       else
          aaa = -dum - dqsdt*(-uzpl(i,k)*g*inv_cp)
       endif

       xx  = max(1.e-8,xx)   ! set lower bound on xx to prevent division by zero
       oxx = 1./xx
...
!       predict_supersaturation2: if (log_predictsSsat) then
...
!       endif predict_supersaturation2
713     continue
...
! vapor
!  to be added (similar to those below) to prevent deposition/condensation to remove qv
!  to the point of subsaturation or negative value
...
!      if (qr(i,k).lt.qsmall .or. nr(i,k).lt.nsmall) then                         !(for consideration)
          qv(i,k) = qv(i,k) + qr(i,k)
          th(i,k) = th(i,k) - th(i,k)/t(i,k)*qr(i,k)*xxls(i,k)*inv_cp
          qr(i,k) = 0.
          nr(i,k) = 0.
...
!         if (qitot(i,k,iice).lt.qsmall .or. nitot(i,k,iice).lt.nsmall) then      !(for consideration)
             qv(i,k) = qv(i,k) + qitot(i,k,iice)
             th(i,k) = th(i,k) - th(i,k)/t(i,k)*qitot(i,k,iice)*xxls(i,k)*inv_cp
             qitot(i,k,iice) = 0.
             nitot(i,k,iice) = 0.
...
555 continue
    ...
    if (debug_ON) call check_values(qv,T,qc,qr,nr,qitot,qirim,nitot,birim,i,it,.true.,debug_ABORT,300)
...
!   if (debug_ON) call check_values(qv,T,qc,qr,nr,qitot,qirim,nitot,birim,i,it,.true.,debug_ABORT,400)
...
!   if (debug_ON) call check_values(qv,T,qc,qr,nr,qitot,qirim,nitot,birim,i,it,.true.,debug_ABORT,500)
    ...
    ! rain:
       if (qr(i,k).ge.qsmall) then
        ...
        ! ze_rain(i,k) = n0r(i,k)*720./lamr(i,k)**3/lamr(i,k)**3/lamr(i,k)
          ...
          ze_rain(i,k) = max(ze_rain(i,k),1.e-28)
       else
          qv(i,k) = qv(i,k)+qr(i,k)
          th(i,k) = th(i,k)-th(i,k)/t(i,k)*qr(i,k)*xxls(i,k)*inv_cp
          qr(i,k) = 0.
...
!                                         qirim(i,k,iice),zitot(i,k,iice),rhop,100)  !future (3-moment)
  ...
  ! note that reflectivity from lookup table is normalized, so we need to multiply by N
             ...
             ze_ice(i,k) = ze_ice(i,k) + 0.1892*f1pr13*nitot(i,k,iice)   ! sum contribution from each ice category (note: 0.1892 = 0.176/0.93)
             ze_ice(i,k) = max(ze_ice(i,k),1.e-28)
...
!.....................................................
    ...
    if (debug_ON) call check_values(qv,T,qc,qr,nr,qitot,qirim,nitot,birim,i,it,.true.,debug_ABORT,900)
...
!-- for "relaxed" testing (specifically, to avoid trapping understandable Qi-Ni values after microphysics source/sink section:
           if (.not.(Qitot(i,k,iice) >= x_low .and. Qitot(i,k,iice) < Q_high .and.       &
               ...
               .not.(Qitot(i,k,iice)<1.e-6 .and. Nitot(i,k,iice)<0.)      ) then
!==
